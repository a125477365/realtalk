...
- **`/tts/speak` 是「缓存优先」：先查 Redis，命中就直接返回缓存音频、不再合成；只有取不到才现合成并写回。** 查缓存、（按需）合成、返回都在这同一个接口的同一次请求里完成——没有"生成接口 + 按 id 二次拉取接口"之分，App 也从不直接读 Redis（音频始终在 HTTP 响应体 / WS 帧里直接拿到）。
- **Redis 只是后端内部的「同句去重缓存」**，键是内容哈希，命中条件是「音色+格式+文本」三者完全一致且 **TTL（默认 5 分钟）**内有人合成过。不可能取到别的语音。
- **手工触发式**（HTTP）走缓存；**沉浸式**（WS）完全不碰 Redis（每句现合成、随流推一次）。
