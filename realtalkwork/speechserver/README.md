...
| 卡片 | Base URL 填 | 效果 |
|---|---|---|
| 语音转写 ASR | `http://<本机>:9100/v1` | 沉浸式/私教/上传语音全走本地转写 |
| 语音合成 TTS | `http://<本机>:9100/v1`（格式选 wav） | 所有朗读走本地合成 |
| 模型（对话/评分/私教） | `http://<本机>:9100/v1` | 对话/评分/指导走本地 LLM |
| 场景生成模型（可单独设） | 本地或云端任意 OpenAI 兼容地址 | 长文场景生成可独立用更强的云端模型 |
| 高级会员实时语音 | 保持 OpenAI / GLM 不变（也可填 `ws://<本机>:9100/v1/realtime`） | 本地失败时高级会员照常用 OpenAI |
